...
export default function Projects({ onNavigate, selectedProjectSlug, setSelectedProjectSlug }: ProjectsProps) {
  ...
  const selectProject = (slug: string) => {
    ...
    if (projectsEl) {
      const yOffset = -80; // offset of header
      const y = projectsEl.getBoundingClientRect().top + window.scrollY + yOffset;
  ...
  return (
    <section id="projects" className="relative py-24 z-10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Dynamic Project Details Overlay or Masonry View */}
        <AnimatePresence mode="wait">
          {activeProject ? (
            <div key="details-view">
              <ProjectDetails
